...
const items = [[{
  ...
  to: '/control/permissions/users/create'
              ...
              <div v-for="(day, index) in labActivityData.days" :key="index" class="flex flex-col items-center gap-2 flex-1">
                <div class="flex flex-col gap-1 w-full">
                  <div 
                    ...
                    :style="{ height: `${((labActivityData.completed[index] || 0) / 25) * 200}px` }"
                  ></div>
                  <div 
                    class="bg-green-500 rounded-b" 
                    :style="{ height: `${((labActivityData.started[index] || 0) / 25) * 200}px` }"
                  ></div>
                </div>
              ...
              <div class="relative w-48 h-48">
                <svg class="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                  ...
                  <circle
                    ...
                    r="40"
                    fill="none"
                    :stroke="(labPerformanceData[0] || { color: '#10b981' }).color"
                    stroke-width="8"
                    :stroke-dasharray="`${((labPerformanceData[0] || { value: 45 }).value) * 2.51} 251`"
                    stroke-linecap="round"
                  />
                </svg>
                <div class="absolute inset-0 flex items-center justify-center">
                  <div class="text-center">
                    <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ (labPerformanceData[0] || { value: 45 }).value }}%</div>
                    <div class="text-sm text-gray-500">{{ (labPerformanceData[0] || { name: 'Отлично' }).name }}</div>
                  </div>
                </div>
              </div>
            </div>
